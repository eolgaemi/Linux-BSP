...
static int gpioLed[GPIOCNT] = {6,7,8,9,10,11,12,13};
static int gpioKey[GPIOCNT] = {16,17,18,19,20,21,22,23};
...
ssize_t ledkey_write (struct file *filp, const char *buf, size_t count, loff_t *f_pos)
{
	...
	ret = get_user(kbuf,buf);
	//ret=copy_from_user(buf,&kbuf,count);
	if(ret<0)
		return -EFAULT;
	gpioLedSet(kbuf);
    return count;
